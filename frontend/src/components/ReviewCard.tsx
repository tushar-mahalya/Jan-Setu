...
export default function ReviewCard({ draft, onReplacePhoto, isReplacingPhoto, onUpdateReview, isUpdatingReview, onConfirm, isConfirming }: Props) {
  ...
  return <article className="review-card" aria-labelledby="review-title">
    ...
    {mismatch && <div className="review-card__warning" role="alert"><strong>{t.mismatchTitle}</strong><p>{t.mismatchBody}</p>{!showPhotoPicker ? <div className="review-card__actions"><button type="button" className="btn btn--secondary" onClick={() => setShowPhotoPicker(true)}>{t.chooseAnotherPhoto}</button></div> : <div className="review-card__photo-retry"><PhotoUpload value={replacementPhoto} onChange={setReplacementPhoto} /><button type="button" className="app-button app-button--teal" disabled={!replacementPhoto || isReplacingPhoto} onClick={() => replacementPhoto && onReplacePhoto(replacementPhoto)}>{isReplacingPhoto ? t.checkingPhoto : t.useThisPhoto}</button></div>}</div>}
