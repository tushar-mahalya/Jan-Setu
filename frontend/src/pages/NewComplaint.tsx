...
export default function NewComplaint() {
  ...
  return <div className="wizard-page">
    ...
    <div className="wizard-stage" key={step}>
      ...
      {step === 3 && draft && <ReviewCard draft={draft} onReplacePhoto={(file) => replace.mutate(file)} isReplacingPhoto={replace.isPending} onConfirm={() => confirm.mutate()} isConfirming={confirm.isPending} />}
    </div>
    {(error || create.isError || replace.isError || confirm.isError) && <p className="app-error wizard-error" role="alert">{error ?? ((create.error || replace.error || confirm.error) instanceof ApiError ? (create.error || replace.error || confirm.error as ApiError).message : "Could not continue. Your entries are still here—please try again.")}</p>}
