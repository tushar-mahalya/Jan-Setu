...
from sqlalchemy import DateTime, ForeignKey, String, Text, desc, select
    ...
    *,
    ...
    statement = (
        ...
        .on_conflict_do_update(
            ...
            set_={"profile_name": profile_name, "updated_at": utc_now()},
        )
        .returning(Contact)
    )
...
async def store_incoming_messages(
    ...
    for incoming in messages:
        ...
        existing = await session.execute(
            select(WhatsAppMessage).where(WhatsAppMessage.meta_message_id == incoming.meta_message_id)
        )
        stored_messages.append(
            StoredIncomingMessage(message=existing.scalar_one(), wa_id=incoming.wa_id, created=False)
        )
